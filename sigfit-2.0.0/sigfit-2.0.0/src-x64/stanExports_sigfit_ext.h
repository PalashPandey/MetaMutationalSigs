...
class model_sigfit_ext
  : public stan::model::model_base_crtp<model_sigfit_ext> {
private:
        int family;
        int C;
        int S;
...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        try {
            ...
            context__.validate_dims("data initialization", "kappa", "vector_d", context__.to_vec(G,S));
            kappa = std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> >(G, Eigen::Matrix<double, Eigen::Dynamic, 1>(S));
            ...
            context__.validate_dims("data initialization", "alpha", "vector_d", context__.to_vec(S,C));
            alpha = std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> >(S, Eigen::Matrix<double, Eigen::Dynamic, 1>(C));
        ...
        try {
            ...
            for (int g = 1; g <= G; ++g) {
                ...
                if (as_bool(logical_eq(dpp, 1))) {
                    ...
                    lp_accum__.add(dirichlet_log<propto__>(get_base1(exposures_raw, g, "exposures_raw", 1), get_base1(kappa, g, "kappa", 1)));
                }
            }
            current_statement_begin__ = 158;
            for (int s = 1; s <= S; ++s) {
                current_statement_begin__ = 160;
                lp_accum__.add(dirichlet_log<propto__>(get_base1(signatures, s, "signatures", 1), get_base1(alpha, s, "alpha", 1)));
            }
